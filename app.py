...
@app.route('/')
def home():
    return "<h1>Woodeng BUY-ONLY Tracker 24/7</h1><p>Ne track que les achats – <a href='/health'>/health</a></p>"

@app.route('/health')
def health():
    return jsonify({"status": "alive", "bot": "Woodeng BUY-ONLY Tracker"}), 200
# ======================================================================
...
ALL_CHAT_IDS = [int(CHAT_ID)] if CHAT_ID.isdigit() else []
ALL_CHAT_IDS.extend([int(g) for g in GROUP_IDS if g.isdigit()])

HELIUS_RPC_URL = os.environ.get("WOODENG_API_URL", "https://mainnet.helius-rpc.com/?api-key=")
...
async def send_transaction_alert(bot: Bot, tx_sig: str, tx_data: dict, session: aiohttp.ClientSession):
    ...
    if changes["woodeng_spent"] <= 0:
        return  # ← IGNORE LES VENTES ET BURNS
    ...
    woodeng_text = format_amount(changes["woodeng_spent"])
    ...
    for chat_id in ALL_CHAT_IDS:
        try:
            if first_image:
                ...
                try:
                    # Télécharge l'image et envoie comme fichier (100 % affichage)
                    async with session.get(first_image, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        if r.status == 200:
                            img_data = await r.read()
                            img_file = io.BytesIO(img_data)
                            img_file.name = "soundmeme.jpg"
                            await bot.send_photo(chat_id=chat_id, photo=img_file, caption=message, parse_mode=ParseMode.MARKDOWN)
