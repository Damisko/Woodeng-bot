...
@app.route('/health')
def health():
    return jsonify({"status": "OK", "bot": "Woodeng BUY-ONLY"}), 200
# ======================================================================
...
IPFS_GATEWAY = "https://gateway.pinata.cloud/ipfs/"  # le plus rapide et fiable
# ======================================================================

sent_txs = set()

def ipfs_to_https(url: str) -> str:
    if not url:
        return None
...
async def get_real_media(mint: str, session) -> dict:
    """Récupère image + audio depuis le vrai JSON IPFS (comme ton exemple)"""
    try:
        async with session.post(HELIUS_RPC, json={"jsonrpc":"2.0","id":1,"method":"getAsset","params":[mint]}) as r:
            ...
            # Télécharge le vrai JSON metadata
            json_url = ipfs_to_https(uri)
            if not json_url: return {}

            async with session.get(json_url) as r2:
                if r2.status != 200: return {}
                meta_json = await r2.json()

            name = meta_json.get("name", mint[:8])
            image = ipfs_to_https(meta_json.get("image"))
            audio = ipfs_to_https(meta_json.get("animation_url") or meta_json.get("external_url"))

            return {"name": name, "image": image, "audio": audio}
    except:
        return {"name": mint[:8], "image": None, "audio": None}

async def is_real_buy(tx) -> bool:
    try:
...
async def send_alert(bot: Bot, sig: str, session):
    if sig in sent_txs and return
    sent_txs.add(sig)
    ...
    # Trouve le premier sound meme acheté
    first_mint = None
    try:
        pre = {(b["owner"], b["mint"]): float(b["uiTokenAmount"]["uiAmount"] or 0)
               for b in tx["meta"].get("preTokenBalances", [])}
        ...
        for k, pre_amt in pre.items():
            ...
            if post.get(k, 0) > pre_amt and not first_mint:
                first_mint = k[1]
    except: spent = 0

    if not first_mint or spent <= 0: return

    # Récupère image + audio du sound meme
    media = await get_real_media(first_mint, session)
